  ...
  def basicGroupRepository[K, C, S](name: String, createRepo: (AppRepository, PodRepository, Int) => GroupRepository): Unit = {
    name should {
      ...
      "store new apps when storing the root" in {
        ...
        val newRoot = root.updateApps(PathId.root, _ => apps.iterator.map(app => app.id -> app).toSeq, root.version)
      ...
      "not store the group if updating apps fails" in {
        ...
        val newRoot = root.updateApps(PathId.root, apps = _ => apps.iterator.map(app => app.id -> app).toSeq, root.version)
