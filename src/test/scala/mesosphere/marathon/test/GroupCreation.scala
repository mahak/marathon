...
trait GroupCreation {
  def createRootGroup(
    ...
    val group = RootGroup(apps, pods, groups.iterator.map(group => group.id -> group).toSeq, dependencies, RootGroup.NewGroupStrategy.fromConfig(newGroupEnforceRole), version)
  ...
  def createGroup(
    ...
    val groupsById: Map[AbsolutePathId, Group] = groups.iterator.map(group => group.id -> group).toSeq
    val group = Group(
      id,
      apps,
      pods,
